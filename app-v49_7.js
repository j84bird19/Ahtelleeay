...
function setInvoiceStatusFromControl(id,status,mode='client'){let inv=state.invoices.find(i=>i.id===id);if(!inv)return;status=String(status||'').toUpperCase();if(status==='PAID'){createBankPaymentForInvoice(inv,invoiceBalance(inv),'invoice-status');}
 else if(status==='UNPAID'){removeAutoInvoicePayments(id);inv._manualPaidBase=0;inv.paid=0;reconcileInvoiceBankPayments(id);}
 else if(status==='PARTIAL'){if(normalizeCents(inv.paid)<=0){inv._manualPaidBase=0;inv.paid=0;}normalizeInvoice(inv,{skipSupplySync:true});}
...
function renderSavingsGoalsPanel(){
  ...
  let rows=sortedGoals.map(g=>{
    ...
    return `<div class="goalCard priority${priority}"><div class="goalTop"><div><b>${escapeHtml(g.name||'Savings Goal')}</b><small>${escapeHtml(g.priority||'Medium')} Priority • Remaining ${money(remaining)}</small></div><button class="delete smallBtn" type="button" onclick="deleteSavingsGoal('${g.id}')">Delete</button></div><div class="goalProgress"><span style="width:${pct}%"></span></div><div class="three goalInputs"><div><label>Total Needed</label><input type="number" step="0.01" value="${Number(g.totalNeeded||0)}" oninput="updateSavingsGoal('${g.id}','totalNeeded',this.value)"></div><div><label>Saved Toward</label><input type="number" step="0.01" value="${Number(g.amountSaved||0)}" oninput="updateSavingsGoal('${g.id}','amountSaved',this.value)"></div><div><label>Priority</label><select onchange="updateSavingsGoal('${g.id}','priority',this.value)">${savingsGoalPriorityOptions(g.priority||'Medium')}</select></div></div><p class="note">${pct}% funded • ${money(g.amountSaved||0)} of ${money(g.totalNeeded||0)}</p></div>`;
  }).join('');
...
if('serviceWorker'in navigator){
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('./service-worker.js?v=49.7');
      reg.update();
    }catch(e){}
  });
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=49.7').then(()=>updatePwaStatus()).catch(()=>updatePwaStatus()));
