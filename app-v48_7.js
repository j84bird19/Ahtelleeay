...
function renderScratchPopover(menu){
  ...
  if(menu==='style'){
    return `<div class="scratchPopover studioPopover stylePopover upgradedStyle"><label class="studioColorPick"><span>Color</span><input class="colorWheelInput" type="color" value="${escapeHtml(sp.color)}" oninput="setScratchColor(this.value)"><i class="colorSwatch" style="background:${escapeHtml(sp.color)}"></i></label><label><span>Size</span><input type="range" min="2" max="48" value="${Number(sp.size||6)}" oninput="setScratchSize(this.value);this.nextElementSibling.textContent=this.value+'px'"><b>${Number(sp.size||6)}px</b></label><label><span>Opacity</span><input type="range" min="5" max="100" value="${opacityPct}" oninput="setScratchOpacity(this.value/100);this.nextElementSibling.textContent=this.value+'%'"><b>${opacityPct}%</b></label></div>`;
  }
  if(menu==='image'){
...
function initScratchPad(){
  ...
  const drawSegment=(a,b,c)=>{
    ctx.save();resetTransform();ctx.lineCap='round';ctx.lineJoin='round';ctx.globalCompositeOperation=state.scratchPad.tool==='eraser'?'destination-out':'source-over';ctx.strokeStyle=state.scratchPad.color;ctx.globalAlpha=state.scratchPad.tool==='eraser'?1:Number(state.scratchPad.opacity||1);
...
if('serviceWorker'in navigator){
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('./service-worker.js?v=48.7');
      reg.update();
    }catch(e){}
  });
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=48.7').then(()=>updatePwaStatus()).catch(()=>updatePwaStatus()));
