...
function recalcSupplyLine(line){
 ...
 line.unit=meta.unit;
 line.pricePerUnit=toNumber(meta.pricePerUnit,0);
 if(line.pricePerUnit>0){line.amount=normalizeMoneyNumber(line.qty*line.pricePerUnit);}
 else{line.amount=normalizeMoneyNumber(line.amount||0);}
 if(meta.item){line.supplyId=meta.item.id;line.name=meta.item.name;}
 return line;
}
...
function normalizeMoneyNumber(value, places=2){
 return Number(roundMoney(toNumber(value,0),places));
}
...
function normalizeInvoice(inv,opts={}){if(!inv)return null;if(!Array.isArray(inv.services))inv.services=[];if(!Array.isArray(inv.supplies))inv.supplies=[];inv.services.forEach(s=>{s.amount=normalizeMoneyNumber(s.amount||0);});inv.supplies.forEach(recalcSupplyLine);inv.total=normalizeMoneyNumber([...(inv.services||[]),...(inv.supplies||[])].reduce((a,l)=>a+toNumber(l.amount,0),0));inv.paid=normalizeMoneyNumber(inv.paid||0);inv.balance=normalizeMoneyNumber(Math.max(inv.total-inv.paid,0));inv.status=getInvoiceStatus(inv).toLowerCase();if(!opts.skipSupplySync)recalcAllSupplyRemaining({silent:true});return inv;}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=48.3').then(()=>updatePwaStatus()).catch(()=>updatePwaStatus()));
