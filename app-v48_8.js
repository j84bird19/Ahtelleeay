...
function attachSupplyPhoto(e,id){let file=e.target.files[0];if(!file)return;let reader=new FileReader();reader.onload=()=>{let item=state.supplyItems[id];if(item){item.photo=reader.result;save('supply-photo');render()}};reader.readAsDataURL(file)}
...
function renderStudioPage(){
  ensureScratchPad();
  const count=state.scratchPad.gallery?.length||0;
  return `<div class="studioStage"><div class="studioHero"><div><span class="studioKicker">Ahtelleeay Studio</span><h2>Creative Workspace</h2><p>Sketch concepts, mark up photos, save client ideas, and keep visual notes in one polished studio space.</p></div><div class="studioBadge"><b>${count}</b><span>saved</span></div></div>${renderScratchModule()}</div>`;
}
...
function ensureScratchPad(){
  ...
  if(state.scratchPad.opacity===undefined||state.scratchPad.opacity===null)state.scratchPad.opacity=1;
  state.scratchPad.opacity=Math.max(0.05,Math.min(1,Number(state.scratchPad.opacity||1)));
  if(!state.scratchPad.menu)state.scratchPad.menu='';
}
function renderScratchModule(){
  ensureScratchPad();
  const sp=state.scratchPad;
  const padActive=sp.activeTab!=='gallery';
  return `<div class="scratchModule studioPanel" id="calendarScratchModule"><div class="scratchTopBar"><div><h3>Sketchbook</h3><p class="note">Draw, mark up photos, save concepts, and attach ideas to clients.</p></div><div class="scratchMiniTabs"><button type="button" class="miniFolderTab ${padActive?'active':''}" onclick="setScratchTab('pad')">Pad</button><button type="button" class="miniFolderTab ${!padActive?'active':''}" onclick="setScratchTab('gallery')">Gallery</button></div></div>${padActive?renderScratchPadView():renderScratchGalleryView()}</div>`;
}
function renderScratchPadView(){
  ensureScratchPad();
  const sp=state.scratchPad;
  const menu=sp.menu||'';
  const drawLabel=sp.tool==='eraser'?'Eraser':'Pencil';
  ...
  const opacityPct=Math.round(Number(sp.opacity||1)*100);
  return `<div class="scratchPadWrap studioSketchDesk"><div class="studioToolShelf"><button type="button" class="toolBtn ${['pencil','eraser'].includes(sp.tool)?'activeTool':''}" onclick="toggleScratchMenu('draw')"><span>✎</span><b>${drawLabel}</b></button><button type="button" class="toolBtn ${['line','circle','square','triangle'].includes(sp.tool)?'activeTool':''}" onclick="toggleScratchMenu('shape')"><span>◇</span><b>${shapeLabel}</b></button><button type="button" class="toolBtn ${menu==='style'?'activeTool':''}" onclick="toggleScratchMenu('style')"><span>◐</span><b>Style</b></button><button type="button" class="toolBtn ${menu==='image'?'activeTool':''}" onclick="toggleScratchMenu('image')"><span>▣</span><b>Image</b></button><button type="button" class="toolBtn" onclick="undoScratch()"><span>↶</span><b>Undo</b></button></div>${renderScratchPopover(menu)}<div class="scratchWorkSurface"><div class="scratchCanvasFrame"><canvas id="scratchCanvas" class="scratchCanvas"></canvas><div id="scratchHint" class="scratchHint">${sp.canvasData?'':'Start sketching, or add a yard/photo image to mark up.'}</div></div><div class="scratchInspector"><div class="inspectorSwatch" style="background:${escapeHtml(sp.color)}"></div><div><b>${escapeHtml(sp.tool)}</b><small>${Number(sp.size||6)}px • ${opacityPct}% opacity</small></div></div></div><div class="scratchActionsCompact studioActionBar"><button type="button" class="save" onclick="saveScratchToGallery()">Save to Gallery</button><button type="button" onclick="clearScratchPad()">Clear</button><button type="button" onclick="shareScratchPad()">Share</button></div></div>`;
}
function renderScratchPopover(menu){
  ...
  const opacityPct=Math.round(Number(sp.opacity||1)*100);
  if(menu==='draw'){
    return `<div class="scratchPopover studioPopover"><button type="button" class="${sp.tool==='pencil'?'activeTool':''}" onclick="setScratchTool('pencil')">✎ Pencil</button><button type="button" class="${sp.tool==='eraser'?'activeTool':''}" onclick="setScratchTool('eraser')">⌫ Eraser</button></div>`;
  }
  if(menu==='shape'){
    return `<div class="scratchPopover studioPopover"><button type="button" class="${sp.tool==='line'?'activeTool':''}" onclick="setScratchTool('line')">╱ Line</button><button type="button" class="${sp.tool==='circle'?'activeTool':''}" onclick="setScratchTool('circle')">○ Circle</button><button type="button" class="${sp.tool==='square'?'activeTool':''}" onclick="setScratchTool('square')">□ Square</button><button type="button" class="${sp.tool==='triangle'?'activeTool':''}" onclick="setScratchTool('triangle')">△ Triangle</button></div>`;
  }
  if(menu==='style'){
    const colors=['#111111','#ffffff','#ef4444','#f97316','#facc15','#22c55e','#06b6d4','#3b82f6','#8b5cf6','#ec4899','#8b5a2b','#9ca3af'];
    return `<div class="scratchPopover studioPopover stylePopover upgradedStyle"><div class="studioStyleBlock"><span>Color</span><div class="studioPalette">${colors.map(c=>`<button type="button" class="colorChip ${String(sp.color).toLowerCase()===c?'activeColor':''}" style="background:${c}" onclick="setScratchColor('${c}')" aria-label="Color ${c}"></button>`).join('')}</div><div class="customColorRow"><i class="colorSwatch" style="background:${escapeHtml(sp.color)}"></i><input id="customScratchColor" value="${escapeHtml(sp.color)}" maxlength="7" inputmode="text" oninput="previewScratchColorInput(this.value)" onchange="applyScratchColorInput(this.value)" placeholder="#111111"><button type="button" onclick="applyScratchColorInput(document.getElementById('customScratchColor')?.value)">Set</button></div></div><label><span>Size</span><input type="range" min="2" max="48" value="${Number(sp.size||6)}" oninput="setScratchSize(this.value);this.nextElementSibling.textContent=this.value+'px'"><b>${Number(sp.size||6)}px</b></label><label><span>Opacity</span><input type="range" min="5" max="100" value="${opacityPct}" oninput="setScratchOpacity(this.value/100);this.nextElementSibling.textContent=this.value+'%'"><b>${opacityPct}%</b></label></div>`;
  }
  if(menu==='image'){
    return `<div class="scratchPopover studioPopover imagePopover"><label class="toolBtn uploadBtnCompact">📁 Choose File<input type="file" accept="image/*" onchange="addScratchImage(event)"></label><label class="toolBtn uploadBtnCompact">📷 Take Photo<input type="file" accept="image/*" capture="environment" onchange="addScratchImage(event)"></label></div>`;
  }
  return '';
}
function renderScratchGalleryView(){
  ...
  return `<div class="scratchGallery studioGallery"><div class="galleryHeader"><div><h3>Gallery</h3><p class="note">Saved sketches and photo markups.</p></div><button type="button" onclick="setScratchTab('pad')">New Sketch</button></div><div class="galleryGridPremium">${items.map(g=>`<article class="galleryCardPremium"><img src="${g.image}" class="galleryImg"><div class="galleryInfo"><b>${escapeHtml(g.title||'Sketch')}</b><small>${escapeHtml(g.date||'')}</small><div class="actions"><button type="button" onclick="addSketchToClient('${g.id}')">Add to Client</button><button type="button" onclick="shareGallerySketch('${g.id}')">Share</button><button type="button" class="delete" onclick="deleteGallerySketch('${g.id}')">Delete</button></div></div></article>`).join('')||'<div class="emptyGallery"><b>No saved sketches yet.</b><p class="note">Save a drawing from the Scratch Pad and it will appear here.</p></div>'}</div></div>`;
}
...
function normalizeHexColor(color){
  ...
  if(/^#[0-9a-fA-F]{3}$/.test(color))return ('#'+color.slice(1).split('').map(ch=>ch+ch).join('')).toLowerCase();
  return null;
}
function refreshScratchToolReadout(){
  const sp=state.scratchPad||{};
  document.querySelectorAll('.inspectorSwatch,.colorSwatch').forEach(el=>{el.style.background=sp.color||'#111111'});
  const custom=document.getElementById('customScratchColor');
  if(custom && normalizeHexColor(sp.color))custom.value=sp.color;
}
function setScratchColor(color){
  ensureScratchPad();
  const hex=normalizeHexColor(color);
  if(!hex)return;
  state.scratchPad.color=hex;
  save('scratch-color');
  refreshScratchToolReadout();
}
function previewScratchColorInput(value){
  const hex=normalizeHexColor(value);
  const swatch=document.querySelector('.customColorRow .colorSwatch');
  if(hex&&swatch)swatch.style.background=hex;
}
function applyScratchColorInput(value){
  const hex=normalizeHexColor(value);
  if(!hex){alert('Use a color like #22c55e');return;}
  setScratchColor(hex);
}
function setScratchSize(size){ensureScratchPad();state.scratchPad.size=Number(size||6);save();}
function setScratchOpacity(opacity){ensureScratchPad();state.scratchPad.opacity=Math.max(0.05,Math.min(1,Number(opacity||1)));save();}
...
function initScratchPad(){
  ...
  let drawing=false,start=null,lastPoint=null,lastMid=null,raf=0,queue=[],shapePoint=null,snap=null;
  ...
  const configureStroke=()=>{ctx.save();resetTransform();ctx.lineCap='round';ctx.lineJoin='round';ctx.miterLimit=2;ctx.globalCompositeOperation=state.scratchPad.tool==='eraser'?'destination-out':'source-over';ctx.strokeStyle=state.scratchPad.color||'#111111';ctx.globalAlpha=state.scratchPad.tool==='eraser'?1:Number(state.scratchPad.opacity||1);ctx.lineWidth=Math.max(1,Number(state.scratchPad.size||6)*(state.scratchPad.tool==='eraser'?1.45:1));};
  const drawSmoothPoint=(p)=>{
    configureStroke();
    if(!lastPoint){
      lastPoint=p;
      lastMid=p;
      ...
      ctx.fillStyle=state.scratchPad.tool==='eraser'?'rgba(0,0,0,1)':(state.scratchPad.color||'#111111');
      if(state.scratchPad.tool==='eraser'){ctx.globalCompositeOperation='destination-out'}
      ctx.fill();
      ctx.restore();
      return;
    }
  ...
  const process=()=>{
    raf=0;
    while(queue.length>0)drawSmoothPoint(queue.shift());
  };
  ...
  const shape=(p)=>{ctx.save();resetTransform();ctx.globalCompositeOperation='source-over';ctx.globalAlpha=Number(state.scratchPad.opacity||1);ctx.lineWidth=Number(state.scratchPad.size||6);ctx.strokeStyle=state.scratchPad.color||'#111111';ctx.lineCap='round';ctx.lineJoin='round';ctx.beginPath();const x=start.x,y=start.y,w=p.x-start.x,h=p.y-start.y;if(state.scratchPad.tool==='line'){ctx.moveTo(x,y);ctx.lineTo(p.x,p.y)}else if(state.scratchPad.tool==='circle'){ctx.ellipse(x+w/2,y+h/2,Math.abs(w/2),Math.abs(h/2),0,0,Math.PI*2)}else if(state.scratchPad.tool==='square'){ctx.roundRect?ctx.roundRect(x,y,w,h,8):ctx.rect(x,y,w,h)}else if(state.scratchPad.tool==='triangle'){ctx.moveTo(x+w/2,y);ctx.lineTo(p.x,p.y);ctx.lineTo(x,y+h);ctx.closePath()}ctx.stroke();ctx.restore();};
  const preview=()=>{const p=shapePoint;if(!p)return;redrawSnap(()=>shape(p));};

  canvas.onpointerdown=e=>{e.preventDefault();canvas.setPointerCapture?.(e.pointerId);drawing=true;start=point(e);lastPoint=null;lastMid=null;queue=[];shapePoint=null;pushUndo();snap=snapshot();if(['pencil','eraser'].includes(state.scratchPad.tool)){queue.push(start);if(!raf)raf=requestAnimationFrame(process)}};
  ...
  const finish=()=>{if(!drawing)return;drawing=false;if(raf){cancelAnimationFrame(raf);raf=0}if(queue.length)process();if(['pencil','eraser'].includes(state.scratchPad.tool))flushStrokeEnd();if(shapePoint&&!['pencil','eraser'].includes(state.scratchPad.tool))preview();commit()};
...
if('serviceWorker'in navigator){
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('./service-worker.js?v=48.8');
      reg.update();
    }catch(e){}
  });
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=48.8').then(()=>updatePwaStatus()).catch(()=>updatePwaStatus()));
