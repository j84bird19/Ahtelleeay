const KEY='bird_planner_v27_schedule_day_view';
...
const now=new Date();
let state=JSON.parse(localStorage.getItem(KEY)||'null')||{section:'schedule',tabs:{schedule:'calendar',clients:'directory',supplies:'list',banking:'accounts'},notes:{},year:now.getFullYear(),month:now.getMonth(),selectedDate:dateKey(now),scheduleView:'month',calendarData:{},clients:{},selectedClient:'',invoices:[],invoiceCounter:1,services:['Weed Eating','Mowing','Painting','Leaf Blowing','Yard Clean','General Labor','Debris Removal','Gutters','Handyman','Other'],supplies:['Gas','Oil','String','Other'],supplyItems:{},selectedSupplyId:'',supplyReceipts:[],supplyCounter:1};
function ensureCollections(){if(!state.clients)state.clients={};if(!state.invoices)state.invoices=[];if(!state.invoiceCounter)state.invoiceCounter=1;if(!state.services)state.services=['Weed Eating','Mowing','Painting','Leaf Blowing','Yard Clean','General Labor','Debris Removal','Gutters','Handyman','Other'];if(!state.supplies)state.supplies=['Gas','Oil','String','Other']}
...
function renderScheduleCalendar(){
 ...
 let y=state.year||now.getFullYear(),m=state.month??now.getMonth();
 ...
 for(let d=1;d<=total;d++){
   let k=makeKey(y,m,d),data=ensureDay(k),agenda=data.agenda||[];
   let previews=agenda.slice(0,3).map(item=>`<div class="preview ${item.canceled?'canceled':''}">${formatTime(item.time)} ${escapeHtml(item.title||'')}</div>`).join('');
   if(agenda.length>3)previews+=`<div class="preview">+${agenda.length-3} more</div>`;
   grid+=`<div class="calDay ${k===state.selectedDate?'selected':''} ${k===todayKey?'today':''}" onclick="openScheduleDay('${k}')"><div class="dayNum">${d}</div>${previews}</div>`;
 }
 ...
 <div class="calendarGrid monthOnly">${grid}</div>`;
}


function renderScheduleDayView(){
 let selectedData=ensureDay(state.selectedDate);
 sortAgenda(selectedData.agenda);
 let selectedDateObj=new Date(state.selectedDate+'T12:00:00');
 let title=selectedDateObj.toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric',year:'numeric'});
 return `<div class="dayView">
   <div class="titleRow">
     <div><h2>${title}</h2><p>Jobs and events in time order.</p></div>
     <button onclick="backToMonthView()">‹ Month</button>
   </div>
   <div class="dayAgendaList">
     ${(selectedData.agenda||[]).map((item,idx)=>dayAgendaRow(item,idx)).join('')||'<p class="note">No jobs or events scheduled for this day.</p>'}
   </div>
   <div class="dayAddBar">
     <button class="save" onclick="openDayJobForm()">+ New Job</button>
     <button class="save" onclick="openDayEventForm()">+ New Event</button>
   </div>
 </div>`;
}
function dayAgendaRow(item,idx){
 let type=item.type||'quick';
 let detail=item.client?` — ${escapeHtml(item.client)}`:(item.location?` — ${escapeHtml(item.location)}`:'');
 return `<div class="dayAgendaItem ${item.canceled?'canceled':''}">
   <div class="dayTime">${formatTime(item.time)||'—'}</div>
   <div class="dayInfo"><b>${escapeHtml(item.title||'Untitled')}</b><small>${type}${detail}</small></div>
   <div class="dayActions"><button onclick="toggleCancel(${idx})">×</button></div>
 </div>`;
}
function openScheduleDay(k){
 state.selectedDate=k;
 let p=parseKey(k);state.year=p.y;state.month=p.m;
 state.scheduleView='day';
 save();render();
}
function backToMonthView(){state.scheduleView='month';save();render()}
function openDayJobForm(){state.tabs.schedule='newJob';state.scheduleReturn='day';save();render()}
function openDayEventForm(){state.tabs.schedule='newEvent';state.scheduleReturn='day';save();render()}
...
function clientHasRealData(name){
 let c=state.clients?.[name]||{};
 if((c.phone||'').trim())return true;
 if((c.address||'').trim())return true;
 if((c.notes||'').trim())return true;
 if(jobsForClient(name).length)return true;
 if((state.invoices||[]).some(inv=>inv.client===name))return true;
 return false;
}
function cleanupPartialClients(){
 if(!state.clients)return;
 Object.keys(state.clients).forEach(name=>{
   let clean=name.trim();
   if(clean!==name){
     if(!state.clients[clean])state.clients[clean]=state.clients[name];
     delete state.clients[name];
   }
 });
 let names=Object.keys(state.clients);
 let toDelete=[];
 names.forEach(name=>{
   let hasData=clientHasRealData(name);
   let isPrefixOfReal=names.some(other=>{
     if(other===name)return false;
     return other.toLowerCase().startsWith(name.toLowerCase()) && clientHasRealData(other);
   });
   if(!hasData && isPrefixOfReal)toDelete.push(name);
 });
 toDelete.forEach(n=>delete state.clients[n]);
}

function renderClientDirectory(){syncClientsFromJobs();cleanupPartialClients();save();let names=Object.keys(state.clients||{}).sort((a,b)=>a.localeCompare(b));return `<div class="titleRow"><div><h2>Client Directory</h2><p>Alphabetical list. Tap a client to open their file.</p></div><div class="actions"><button class="save" onclick="openNewClientJob()">+ Add Client</button><button onclick="cleanupPartialClients();save();render()">Clean List</button><div class="note">${names.length} clients</div></div></div><div class="clientList">${names.map(n=>{let t=clientTotals(n);return `<div class="clientCard" onclick="openClient('${escapeAttr(n)}')"><b>${escapeHtml(n)}</b><small>${escapeHtml(state.clients[n].phone||'')} ${escapeHtml(state.clients[n].address||'')}</small><br><small>${t.hours.toFixed(2)} hrs • Paid ${money(t.paid)} • Owed ${money(t.balance)}</small></div>`}).join('')||'<p class="note">No clients yet. Save a job with a client name to auto-create one.</p>'}</div>`}
...
function saveSupplyItem(id){
 ...
 item.name=document.getElementById('sName')?.value.trim() || item.name || `Supply ${item.itemNumber}`;
 item.description=document.getElementById('sDesc')?.value || '';
 item.supplier=document.getElementById('sSupplier')?.value || '';
 item.storeItemNumber=document.getElementById('sStoreNumber')?.value || '';
 item.price=document.getElementById('sPrice')?.value || '';
 item.quantityForPrice=document.getElementById('sQtyForPrice')?.value || '';
 item.unit=document.getElementById('sUnit')?.value || '';
...
function getSupplyArray(){
 ensureCollections();
 return Object.values(state.supplyItems||{})
   .filter(item=>item && item.id && (item.name || item.id===state.selectedSupplyId))
   .sort((a,b)=>Number(a.itemNumber||0)-Number(b.itemNumber||0));
}
...
function updateSupplyField(id,key,value){
 ...
 if(!item)return;
 item[key]=value;
 if(key==='name' && value.trim())addSupplyToDb(value.trim());
 save();
}
function calcSupplyUnitPrice(id){
 ...
 if(cost && amount){
   ...
   item.pricePerUnit=cost.toFixed(4);
 }
...
function supplyTotals(id){
 ...
 (state.invoices||[]).forEach(inv=>{
   (inv.supplies||[]).forEach(s=>{
     if(s.supplyId===id || (item.name && s.name===item.name)){
       used+=Number(s.qty||0);
     }
   });
 });

 return{spent:spent,used:used,purchased:purchased};
}
...
function removeClientInvoiceLine(id,type,idx){let inv=state.invoices.find(i=>i.id===id);if(!inv)return;inv[type].splice(idx,1);recalcInvoice(inv);refreshClientInvoiceEmbed(id)}
...
function setCalendarMonth(m){state.month=m;state.selectedDate=makeKey(state.year,m,1);state.scheduleView='month';save();render()}function changeCalendarYear(n){state.year+=n;state.selectedDate=makeKey(state.year,state.month,1);state.scheduleView='month';save();render()}function selectCalendarDay(k){openScheduleDay(k)}
...
function upsertClientFromJob(job){
 if(!state.clients)state.clients={};
 let n=String(job.client||'').trim();
 if(!n)return;
 let existingName=Object.keys(state.clients).find(x=>x.toLowerCase()===n.toLowerCase());
 if(existingName)n=existingName;
 if(!state.clients[n])state.clients[n]={name:n,phone:'',address:'',notes:''};
 if(job.phone)state.clients[n].phone=job.phone;
 if(job.address)state.clients[n].address=job.address;
 if(job.notes&&!state.clients[n].notes)state.clients[n].notes=job.notes;
 cleanupPartialClients();
}
...
function openClient(n){state.selectedClient=n;state.tabs.clients='client';save();render()}function saveClientEdit(){
 let old=state.selectedClient;
 let name=document.getElementById('clientNameEdit').value.trim();
 if(!name)return;
 if(!state.clients)state.clients={};
 if(name!==old){
   delete state.clients[old];
   renameClientInJobs(old,name);
   (state.invoices||[]).forEach(inv=>{if(inv.client===old)inv.client=name});
 }
 state.clients[name]={
   name,
   phone:document.getElementById('clientPhoneEdit').value.trim(),
   address:document.getElementById('clientAddressEdit').value.trim(),
   notes:document.getElementById('clientNotesEdit').value.trim()
 };
 state.selectedClient=name;
 cleanupPartialClients();
 save();
