...
function moneyNumber(value,places=2){return Number(roundMoney(numberOrZero(value),places));}
function calcSupplyUnitPriceValue(item){
  if(!item)return '';
  const cost=numberOrZero(item.price);
  const amount=numberOrZero(item.quantityForPrice);
  return cost>0&&amount>0?roundMoney(cost/amount,4):'';
}
function recalcInvoiceTotalsOnly(inv){
  if(!inv)return;
  if(!Array.isArray(inv.services))inv.services=[];
  if(!Array.isArray(inv.supplies))inv.supplies=[];
  inv.services.forEach(s=>{s.amount=moneyNumber(s.amount); if(s.qty!==undefined)s.qty=numberOrZero(s.qty);});
  inv.supplies.forEach(recalcSupplyLine);
  inv.total=moneyNumber([...(inv.services||[]),...(inv.supplies||[])].reduce((sum,line)=>sum+numberOrZero(line.amount),0));
  inv.paid=moneyNumber(inv.paid);
  inv.status=getInvoiceStatus(inv).toLowerCase();
}
function recalcSupplyRemainingOnly(id){
  const item=state.supplyItems?.[id];
  if(!item)return;
  if(!Array.isArray(item.inventoryLog))item.inventoryLog=[];
  const totals=supplyTotals(id);
  item.quantityRemaining=Math.max(numberOrZero(totals.purchased)-numberOrZero(totals.used),0);
}
function normalizeStateForSave(){
  if(mathSyncLock)return;
  mathSyncLock=true;
  try{
    ensureCollections();
    Object.values(state.supplyItems||{}).forEach(item=>{
      if(!item)return;
      if(!Array.isArray(item.inventoryLog))item.inventoryLog=[];
      item.unit=String(item.unit||'unit').trim()||'unit';
      item.pricePerUnit=calcSupplyUnitPriceValue(item);
      item.quantityRemaining=numberOrZero(item.quantityRemaining);
    });
    (state.invoices||[]).forEach(recalcInvoiceTotalsOnly);
    Object.keys(state.supplyItems||{}).forEach(recalcSupplyRemainingOnly);
    ensureSupplyDbFromNames?.();
  }catch(err){console.warn('Math sync normalization skipped:',err)}
  finally{mathSyncLock=false;}
}
...
function addQuickSupplyFromList(){
 ...
 let price=numberOrZero(document.getElementById('quickSupplyPrice')?.value||0);
 ...
 let id=uid();let number=nextSupplyNumber();
 ...
 state.supplyItems[id]=item;addSupplyToDb(name);save('quick-supply-new');render();
}
...
function updateSupplyField(id,key,value){
 ...
 if(!item)return;
 item[key]=value;
 if(key==='name')addSupplyToDb(value);
 if(['price','quantityForPrice','unit'].includes(key)){
   calcSupplyUnitPrice(id,{silent:true});
   recalcSupplyRemaining(id,{silent:true});
 }
 save('supply-field');
}
function calcSupplyUnitPrice(id,options={}){
 let item=state.supplyItems?.[id];
 if(!item)return;
 item.pricePerUnit=calcSupplyUnitPriceValue(item);
 let el=document.getElementById('sPricePerUnit');
 if(el)el.value=item.pricePerUnit||'';
 if(!options.silent)save('supply-unit-price');
}
...
function deleteSupplyItem(id){ensureCollections();let item=state.supplyItems[id];if(!item)return;let used=(state.invoices||[]).some(inv=>(inv.supplies||[]).some(s=>s.supplyId===id));let msg=used?'This supply is already used on at least one invoice. Delete it from the supply list anyway? Existing invoices will keep their line item history.':'Delete this supply item?';if(!confirm(msg))return;delete state.supplyItems[id];state.supplies=(state.supplies||[]).filter(name=>String(name||'').trim().toLowerCase()!==String(item.name||'').trim().toLowerCase() || findSupplyByName(name));if(state.selectedSupplyId===id)state.selectedSupplyId='';state.section='supplies';state.tabs.supplies='list';save('delete-supply');render()}
function supplyTotals(id){
 let item=state.supplyItems[id];
 if(!item)return{spent:0,used:0,purchased:0};
 let spent=0,used=0,purchased=0;
 (item.inventoryLog||[]).forEach(log=>{purchased+=numberOrZero(log.qty);spent+=numberOrZero(log.cost);});
 (state.supplyReceipts||[]).forEach(r=>{(r.items||[]).forEach(i=>{if(i.supplyId===id){purchased+=numberOrZero(i.qty);spent+=numberOrZero(i.amount);}});});
 (state.invoices||[]).forEach(inv=>{(inv.supplies||[]).forEach(s=>{if(s.supplyId===id || String(s.name||'').trim().toLowerCase()===String(item.name||'').trim().toLowerCase()){used+=numberOrZero(s.qty);}});});
 return{spent,used,purchased};
}
function findSupplyByName(name){ensureCollections();let target=String(name||'').trim().toLowerCase();if(!target)return null;return Object.values(state.supplyItems||{}).find(item=>!item.isDraft&&String(item.name||'').trim().toLowerCase()===target)||null;}
function recalcSupplyRemaining(id,options={}){recalcSupplyRemainingOnly(id);let item=state.supplyItems?.[id];let el=document.getElementById('sRemaining');if(el&&item)el.value=item.quantityRemaining;if(!options.silent)save('supply-remaining');}
function recalcAllSupplyRemaining(options={}){ensureCollections();Object.keys(state.supplyItems||{}).forEach(id=>recalcSupplyRemaining(id,{silent:true}));if(!options.silent)save('all-supply-remaining');}
function previewSupplyRemaining(id,value){let item=state.supplyItems?.[id];if(!item)return;item.quantityRemaining=Math.max(numberOrZero(value),0);save('supply-remaining-preview');}
function commitSupplyRemainingCorrection(id,value){
 let item=readSupplyFormIntoState(id)||state.supplyItems?.[id];
 if(!item)return;
 finalizeSupplyItemName(item);
 calcSupplyUnitPrice(id,{silent:true});
 recalcSupplyRemaining(id,{silent:true});
 const desired=Math.max(numberOrZero(value),0);
 const current=numberOrZero(item.quantityRemaining);
 const delta=desired-current;
 if(Math.abs(delta)>0.0001){
   if(!Array.isArray(item.inventoryLog))item.inventoryLog=[];
   item.inventoryLog.push({id:uid(),type:'physical-correction',qty:delta,date:new Date().toLocaleDateString(),cost:0,note:'Physical count correction'});
 }
 item.isDraft=false;
 recalcSupplyRemaining(id,{silent:true});
 addSupplyToDb(item.name);
 save('supply-physical-correction');
 renderCalendarOnlySoon();
}
function addInventoryToSupply(id){
 let item=readSupplyFormIntoState(id);if(!item)return;
 let qty=numberOrZero(document.getElementById('invAddQty')?.value||0);
 if(!qty || qty<0){alert('Enter a quantity greater than 0.');return;}
 finalizeSupplyItemName(item);calcSupplyUnitPrice(id,{silent:true});
 if(!item.inventoryLog)item.inventoryLog=[];
 item.inventoryLog.push({id:uid(),qty,date:document.getElementById('invAddDate')?.value||new Date().toLocaleDateString(),cost:moneyNumber(numberOrZero(item.pricePerUnit)*qty)});
 item.isDraft=false;recalcSupplyRemaining(id,{silent:true});addSupplyToDb(item.name);save('add-inventory');
 state.section='supplies';state.tabs.supplies='item';state.selectedSupplyId=id;render();
}
...
function recalcInvoice(inv,options={}){recalcInvoiceTotalsOnly(inv);recalcAllSupplyRemaining({silent:true});if(!options.silent)save('invoice-recalc')}
...
function updateInvoiceLine(id,type,idx,value){let inv=state.invoices.find(i=>i.id===id);if(!inv||!inv[type]?.[idx])return;inv[type][idx].amount=moneyNumber(value);if(type==='supplies'){recalcSupplyLine(inv[type][idx]);}recalcInvoice(inv)}
function updateInvoiceSupplyQty(id,idx,value,mode='full'){
 ...
 line.qty=Math.max(numberOrZero(value),0);
 recalcSupplyLine(line);
 recalcInvoice(inv,{silent:true});
 if(oldSupplyId)recalcSupplyRemaining(oldSupplyId,{silent:true});
 if(line.supplyId&&line.supplyId!==oldSupplyId)recalcSupplyRemaining(line.supplyId,{silent:true});
 save('invoice-supply-qty');
...
function autosaveSupplyItemPage(){
 ...
 const hasData=hasAnyValue({name:item.name,description:item.description,supplier:item.supplier,storeItemNumber:item.storeItemNumber,price:item.price,quantityForPrice:item.quantityForPrice});
 calcSupplyUnitPrice(id,{silent:true});
 recalcSupplyRemaining(id,{silent:true});
 if(hasData){item.isDraft=false;finalizeSupplyItemName(item);addSupplyToDb(item.name);}
 save('autosave-supply-item');
 return true;
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=48.2').then(()=>updatePwaStatus()).catch(()=>updatePwaStatus()));
