...
function bankDraftField(key,value){ensureBanking();state.banking.draft[key]=value;if(key==='type'&&!state.banking.draft.category){state.banking.draft.category=value==='expense'?'General':'Job Income'}save('bank-draft')}
function bankTotals(){
  ensureBanking();
...
function renderBankingAccounts(){
  ...
  const recent=state.banking.transactions.slice().sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||''))).slice(0,8);
...
function bankReceiptCard(t){
  return `<div class="invoiceCard bankCard"><b>${escapeHtml(t.description||'Receipt')} — ${money(t.amount)}</b><small>${escapeHtml(t.date||'')} • ${escapeHtml(t.category||'')} • ${escapeHtml(t.method||'')}</small>${t.image?`<img class="photo" src="${t.image}">`:''}<div class="actions"><button class="smallBtn" onclick="openBankTransaction('${t.id}')">Open</button><button class="delete smallBtn" onclick="deleteBankTransaction('${t.id}')">Delete</button></div></div>`;
}
function addBankTransaction(){
  ...
  const tx={id:uid(),type:d.type||'income',date:d.date||dateKey(new Date()),description:String(d.description||'').trim()||bankDefaultDescription(d.type),amount,category:d.category||'Other',method:d.method||'Cash',notes:d.notes||'',image:'',createdAt:new Date().toISOString()};
  state.banking.transactions.push(tx);
  state.banking.draft={type:d.type||'income',date:dateKey(new Date()),description:'',amount:'',category:d.category||'Job Income',method:d.method||'Cash',notes:'',image:''};
  save('bank-add');
  render();
}
function bankDefaultDescription(type){return type==='expense'?'Purchase':type==='transfer'?'Savings Transfer':'Money Received';}
function clearBankDraft(){ensureBanking();state.banking.draft={type:'income',date:dateKey(new Date()),description:'',amount:'',category:'Job Income',method:'Cash',notes:'',image:''};save('bank-clear');render();}
function deleteBankTransaction(id){ensureBanking();if(!confirm('Delete this banking entry?'))return;state.banking.transactions=state.banking.transactions.filter(t=>t.id!==id);save('bank-delete');render();}
