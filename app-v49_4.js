...
function updateClientInvoicePaid(id,value){let inv=state.invoices.find(i=>i.id===id);if(!inv)return;setInvoiceManualPaidTotal(inv,value);save('client-invoice-paid');refreshClientInvoiceEmbed(id)}
function markClientInvoicePaid(id){let inv=state.invoices.find(i=>i.id===id);if(!inv)return;createBankPaymentForInvoice(inv,invoiceBalance(inv),'invoice');refreshClientInvoiceEmbed(id)}
...
function updateInvoicePaid(id,value){let inv=state.invoices.find(i=>i.id===id);if(!inv)return;setInvoiceManualPaidTotal(inv,value);save('invoice-paid');renderInvoiceEditor(id)}
...
/* ===== V49.4 BANKING ↔ INVOICE / SUPPLY SYNC ===== */
function bankLinkedInvoicePaymentTotal(invoiceId){
  ensureBanking();
  return normalizeMoneyNumber(state.banking.transactions.reduce((sum,t)=>{
    if(t.type==='income'&&t.linkedInvoiceId===invoiceId)return sum+normalizeMoneyNumber(t.amount||0,2);
    return sum;
  },0),2);
}
function reconcileInvoiceBankPayments(invoiceId){
  ...
  if(inv._manualPaidBase===undefined || inv._manualPaidBase===null){
    inv._manualPaidBase=Math.max(normalizeMoneyNumber(inv.paid||0,2)-linked,0);
  }
  const base=normalizeMoneyNumber(inv._manualPaidBase||0,2);
  inv.paid=Math.min(normalizeMoneyNumber(base+linked,2),normalizeMoneyNumber(inv.total||0,2));
  normalizeInvoice(inv,{skipSupplySync:true});
}
...
function setInvoiceManualPaidTotal(inv,value){
  if(!inv)return;
  const target=normalizeMoneyNumber(value||0,2);
  const linked=bankLinkedInvoicePaymentTotal(inv.id);
  inv._manualPaidBase=Math.max(target-linked,0);
...
function createBankPaymentForInvoice(inv,amount,source='invoice'){
  ...
  if(!inv)return null;
  normalizeInvoice(inv,{skipSupplySync:true});
  const amt=normalizeMoneyNumber(amount||invoiceBalance(inv),2);
  if(amt<=0)return null;
  if(inv._manualPaidBase===undefined || inv._manualPaidBase===null)inv._manualPaidBase=normalizeMoneyNumber(inv.paid||0,2);
  const tx={
    id:uid(),type:'income',date:dateKey(new Date()),
    description:`Invoice #${inv.number} payment${inv.client?' — '+inv.client:''}`,
    amount:amt,category:'Job Income',method:'Cash',notes:source==='invoice'?'Auto-created when invoice was marked paid.':'Linked from banking entry.',image:'',
    linkedInvoiceId:inv.id,linkedSupplyId:'',autoInvoicePayment:source==='invoice',createdAt:new Date().toISOString()
  };
...
function bankCategories(){return ['Job Income','Supplies','Fuel','Tools','Materials','Labor','Advertising','Office','Food','Travel','Savings','Personal','Other'];}
...
function unpaidInvoiceOptions(selected=''){
  ...
  return (state.invoices||[])
    .filter(inv=>normalizeCents(invoiceBalance(inv))>0)
    .sort((a,b)=>Number(a.number||0)-Number(b.number||0))
    .map(inv=>`<option value="${escapeAttr(inv.id)}" ${selected===inv.id?'selected':''}>#${inv.number} — ${escapeHtml(inv.client||'No Client')} — Balance ${money(invoiceBalance(inv))}</option>`)
    .join('');
}
...
function bankSelectInvoice(invoiceId){
  ...
  if(invoiceId){
    ...
    if(inv){
      ...
      d.amount=invoiceBalance(inv).toFixed(2);
...
function renderBankingAccounts(){
  ...
  const cats=bankCategories().map(c=>`<option value="${escapeHtml(c)}" ${d.category===c?'selected':''}>${escapeHtml(c)}</option>`).join('');
  const methods=bankMethods().map(m=>`<option value="${escapeHtml(m)}" ${d.method===m?'selected':''}>${escapeHtml(m)}</option>`).join('');
  const invoiceLink=d.type==='income'?`<div><label>Link Unpaid Invoice</label><select id="bankLinkedInvoice" onchange="bankSelectInvoice(this.value)"><option value="">None / general income</option>${unpaidInvoiceOptions(d.linkedInvoiceId||'')}</select></div>`:'';
...
function renderBankingTrackers(){
  ...
  return `<div class="titleRow"><div><h2>Money Trackers</h2><p>Totals update from banking entries and invoice payments.</p></div><div class="note">Invoice paid: ${money(invoicePaid)}</div></div><div class="trackers"><div class="tracker">Received<b>${money(t.income)}</b></div><div class="tracker">Spent<b>${money(t.spent)}</b></div><div class="tracker">Saved<b>${money(t.saved)}</b></div><div class="tracker">Available<b>${money(t.spendingBalance)}</b></div></div><div class="box"><h3>Spending By Category</h3>${cats.map(([cat,amt])=>`<div class="bankCatRow"><span>${escapeHtml(cat)}</span><b>${money(amt)}</b></div>`).join('')||'<p class="note">No spending categories yet.</p>'}</div><div class="box"><h3>All Transactions</h3><div class="clientList bankList">${state.banking.transactions.slice().sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||''))).map(bankTransactionCard).join('')||'<p class="note">No transactions yet.</p>'}</div></div>`;
}
...
function addBankTransaction(){
  ...
  const tx={id:uid(),type:d.type||'income',date:d.date||dateKey(new Date()),description:String(d.description||'').trim()||bankDefaultDescription(d.type),amount,category:d.category||'Other',method:d.method||'Cash',notes:d.notes||'',image:'',linkedInvoiceId:d.type==='income'?(d.linkedInvoiceId||''):'',linkedSupplyId:d.type==='expense'?(d.linkedSupplyId||''):'',createdAt:new Date().toISOString()};
  state.banking.transactions.push(tx);
  if(tx.linkedInvoiceId)reconcileInvoiceBankPayments(tx.linkedInvoiceId);
...
function openBankTransaction(id){
  ...
  const cats=bankCategories().map(c=>`<option value="${escapeHtml(c)}" ${t.category===c?'selected':''}>${escapeHtml(c)}</option>`).join('');
...
async function processReceiptPhoto(file){
 const resized=await resizeImageDataUrl(file,1800,.82);
 return await autoCropDocumentDataUrl(resized,1400,.78);
}
...
if('serviceWorker'in navigator){
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('./service-worker.js?v=48.9');
      reg.update();
    }catch(e){}
  });
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=48.9').then(()=>updatePwaStatus()).catch(()=>updatePwaStatus()));
