...
function adminRecordText(records){
  ...
  return records.map(r=>{
    const lines=[`AHTELLEEAY ADMIN RECORD`, `Type: ${r.type}`, `Title: ${r.title}`, `Time: ${new Date(r.createdAt).toLocaleString()}`, `Build: ${r.build||BUILD}`, ''];
...
function emailRecords(records,quiet=false){
  ...
  const subject=encodeURIComponent(`Ahtelleeay Admin Backup Receipt (${records.length})`);
  ...
  if(navigator.share&&!cfg.email){
    navigator.share({title:'Ahtelleeay Admin Backup Receipt',text:adminRecordText(records)}).catch(()=>{});
  }else{
...
function exportFullDatabase(){
  ...
  const payload={app:'Ahtelleeay',format:'ahtelleeay-full-backup',version:1,build:BUILD,exportedAt:new Date().toISOString(),state};
  downloadTextFile(`ahtelleeay-full-backup-${dateKey(new Date())}.json`,JSON.stringify(payload,null,2));
...
function importFullDatabase(event){
  ...
  reader.onload=()=>{
    try{
      ...
      console.error(err);
      alert('Could not import that backup file. Make sure it is an Ahtelleeay JSON backup.');
    }
  };
...
function downloadAdminRecordLog(){
  const cfg=ensureAdminBackup();
  downloadTextFile(`ahtelleeay-admin-record-log-${dateKey(new Date())}.json`,JSON.stringify({app:'Ahtelleeay',build:BUILD,exportedAt:new Date().toISOString(),records:cfg.records},null,2));
}
...
function exportBusinessReport(){const html=reportShell('Business Records Report',businessReportBody());openReportHtml('Business Records Report',html,`ahtelleeay-business-report-${dateKey(new Date())}.html`)}
...
function exportClientReport(){const html=reportShell('Client Report',clientReportBody());openReportHtml('Client Report',html,`ahtelleeay-client-report-${dateKey(new Date())}.html`)}
...
if('serviceWorker'in navigator){
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('./service-worker.js?v=49.11');
      reg.update();
    }catch(e){}
  });
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=49.11').then(()=>updatePwaStatus()).catch(()=>updatePwaStatus()));
