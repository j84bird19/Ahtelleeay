...
function calcSupplyLineAmount(name,qty){
 ...
 return item?normalizeMoneyNumber(getSupplyUnitCost(item)*toNumber(qty,0)):0;
}
function supplyLineMeta(line){
 ...
 let pricePerUnit=item?getSupplyUnitCost(item):toNumber(line?.pricePerUnit,0);
...
function recalcSupplyLine(line){
 ...
 line.amount=normalizeMoneyNumber(line.qty*line.pricePerUnit);
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=48.4').then(()=>updatePwaStatus()).catch(()=>updatePwaStatus()));
